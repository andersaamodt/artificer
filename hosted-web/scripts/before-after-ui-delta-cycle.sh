...
PROBE_SCRIPT="$SCRIPT_DIR/before-after-ui-delta-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-before-after-ui-delta-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-before-after-ui-delta-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  before-after-ui-delta-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  before-after-ui-delta-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "before-after-ui-delta-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  {
    printf '# Before/After UI Delta %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Before/After UI Delta Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="before-after-ui-delta-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="before-after-ui-delta-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
