...
PROBE_SCRIPT="$SCRIPT_DIR/current-api-migration-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-current-api-migration-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-current-api-migration-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  current-api-migration-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  current-api-migration-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "current-api-migration-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  {
    printf '# Current API Migration %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Current API Migration Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="current-api-migration-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="current-api-migration-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
