...
PROBE_SCRIPT="$SCRIPT_DIR/gui-state-recovery-pack-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-gui-state-recovery-pack-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-gui-state-recovery-pack-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  gui-state-recovery-pack-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  gui-state-recovery-pack-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "gui-state-recovery-pack-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
    ...
    while IFS=$(printf '\t') read -r task_id scenario; do
      ...
      if [ -n "$row_json" ] && [ -f "$row_json" ]; then
        ...
        row_payload=$(printf '{"label":%s,"status":"fail","probe_rc":%s,"probe_output":%s}' \
          "$(printf '%s' "$row_label" | jq -Rs '.')" \
          "$probe_rc" \
          "$(printf '%s' "$probe_output" | jq -Rs '.')")
      fi
...
run_transfer() {
  ...
  {
    printf '# GUI State Recovery Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="gui-state-recovery-pack-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="gui-state-recovery-pack-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
