...
PROBE_SCRIPT="$SCRIPT_DIR/system-ops-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-system-ops-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-system-ops-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  system-ops-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  system-ops-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "system-ops-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
