...
PROBE_SCRIPT="$SCRIPT_DIR/multi-service-partial-rollback-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-multi-service-partial-rollback-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-multi-service-partial-rollback-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  multi-service-partial-rollback-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  multi-service-partial-rollback-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "multi-service-partial-rollback-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  {
    printf '# Multi-Service Partial Rollback %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Multi-Service Partial Rollback Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="multi-service-partial-rollback-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="multi-service-partial-rollback-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
