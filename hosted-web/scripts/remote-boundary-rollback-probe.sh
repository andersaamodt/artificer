...
DEFAULT_LABEL="remote-boundary-rollback-probe"
API_SCRIPT="${ARTIFICER_API_SCRIPT:-$SITE_ROOT/cgi/artificer-api}"

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required for remote boundary rollback probe." >&2
  exit 1
fi

usage() {
  cat <<'EOF_USAGE'
Usage: remote-boundary-rollback-probe.sh [--label NAME] [--prompt TEXT] [--prompt-file PATH]

Runs a live bounded remote boundary rollback probe against a demo workspace and checks
whether Artificer can open the bastion tunnel, roll back the private canary target first,
then roll back the private fleet target while keeping rollback intact.
EOF_USAGE
}
...
create_remote_boundary_rollback_workspace() {
  workspace_dir=$1
...
APPROVED_RELEASE=2026.03.22
TUNNEL_READY=0
CANARY_ROLLBACK_READY=0
FLEET_ROLLBACK_READY=0
READ_ONLY=0
ROLLOUT_STATE=partial
EOF_CFG
  cp "$workspace_dir/remote/boundary.env" "$workspace_dir/remote/boundary.env.bak"
  printf '%s\n' '2026.03.22' > "$workspace_dir/state/private.canary.release"
  printf '%s\n' '2026.03.22' > "$workspace_dir/state/private.fleet.release"
...
case "$subcommand" in
  status)
    if [ "$APPROVED_RELEASE" = "$STABLE_RELEASE" ] && [ "$TUNNEL_READY" = "1" ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "boundary_bastion=rollback_ready host=$BASTION_HOST release=$STABLE_RELEASE"
    else
      printf '%s\n' "boundary_bastion=stale host=$BASTION_HOST current=$CURRENT_RELEASE stable=$STABLE_RELEASE"
      printf '%s\n' "expected_fix=APPROVED_RELEASE=$STABLE_RELEASE TUNNEL_READY=1 READ_ONLY=1"
    fi
    ;;
  tunnel)
    if [ "$APPROVED_RELEASE" = "$STABLE_RELEASE" ] && [ "$TUNNEL_READY" = "1" ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "$STABLE_RELEASE" > "$state_file"
      printf '%s\n' "boundary_tunnel=ok host=$BASTION_HOST release=$STABLE_RELEASE"
    else
      printf '%s\n' 'blocked' > "$state_file"
      printf '%s\n' "boundary_tunnel=failed host=$BASTION_HOST release=$STABLE_RELEASE"
      exit 1
    fi
    ;;
  health)
    tunneled=$(cat "$state_file" 2>/dev/null || printf '%s' 'unknown')
    if [ "$tunneled" = "$STABLE_RELEASE" ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "boundary_bastion_health=ok host=$BASTION_HOST release=$STABLE_RELEASE"
      exit 0
    fi
    printf '%s\n' "boundary_bastion_health=failed host=$BASTION_HOST release=$STABLE_RELEASE"
    exit 1
    ;;
  *)
...
case "$subcommand" in
  status)
    current=$(cat "$canary_state_file" 2>/dev/null || printf '%s' 'unknown')
    if [ "$APPROVED_RELEASE" = "$STABLE_RELEASE" ] && [ "$CANARY_ROLLBACK_READY" = "1" ] && [ "$READ_ONLY" = "1" ] && [ "$current" = "$CURRENT_RELEASE" ]; then
      printf '%s\n' "boundary_canary=rollback_ready host=$CANARY_PRIVATE_HOST current=$current stable=$STABLE_RELEASE"
    else
      printf '%s\n' "boundary_canary=stale host=$CANARY_PRIVATE_HOST current=$current stable=$STABLE_RELEASE"
      printf '%s\n' "expected_fix=APPROVED_RELEASE=$STABLE_RELEASE CANARY_ROLLBACK_READY=1 READ_ONLY=1"
    fi
    ;;
  rollback)
    tunneled=$(cat "$tunnel_state_file" 2>/dev/null || printf '%s' 'unknown')
    current=$(cat "$canary_state_file" 2>/dev/null || printf '%s' 'unknown')
    if [ "$APPROVED_RELEASE" = "$STABLE_RELEASE" ] && [ "$CANARY_ROLLBACK_READY" = "1" ] && [ "$READ_ONLY" = "1" ] && [ "$tunneled" = "$STABLE_RELEASE" ] && [ "$current" = "$CURRENT_RELEASE" ]; then
      printf '%s\n' "$STABLE_RELEASE" > "$canary_state_file"
      printf '%s\n' "boundary_canary_rollback=ok host=$CANARY_PRIVATE_HOST release=$STABLE_RELEASE"
    else
      printf '%s\n' 'blocked' > "$canary_state_file"
      printf '%s\n' "boundary_canary_rollback=failed host=$CANARY_PRIVATE_HOST release=$STABLE_RELEASE"
      exit 1
    fi
    ;;
  health)
    current=$(cat "$canary_state_file" 2>/dev/null || printf '%s' 'unknown')
    if [ "$current" = "$STABLE_RELEASE" ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "boundary_canary_health=ok host=$CANARY_PRIVATE_HOST release=$STABLE_RELEASE"
      exit 0
    fi
    printf '%s\n' "boundary_canary_health=failed host=$CANARY_PRIVATE_HOST release=$STABLE_RELEASE"
    exit 1
    ;;
  *)
    echo "usage: ssh-private-canary.sh status|rollback|health" >&2
    exit 2
    ;;
esac
EOF_BIN
...
case "$subcommand" in
  status)
    current=$(cat "$fleet_state_file" 2>/dev/null || printf '%s' 'unknown')
    if [ "$APPROVED_RELEASE" = "$STABLE_RELEASE" ] && [ "$FLEET_ROLLBACK_READY" = "1" ] && [ "$READ_ONLY" = "1" ] && [ "$current" = "$CURRENT_RELEASE" ]; then
      printf '%s\n' "boundary_fleet=rollback_ready host=$FLEET_PRIVATE_HOST current=$current stable=$STABLE_RELEASE"
    else
      printf '%s\n' "boundary_fleet=stale host=$FLEET_PRIVATE_HOST current=$current stable=$STABLE_RELEASE"
      printf '%s\n' "expected_fix=APPROVED_RELEASE=$STABLE_RELEASE FLEET_ROLLBACK_READY=1 READ_ONLY=1"
    fi
    ;;
  rollback)
    canary_release=$(cat "$canary_state_file" 2>/dev/null || printf '%s' 'unknown')
    current=$(cat "$fleet_state_file" 2>/dev/null || printf '%s' 'unknown')
    if [ "$APPROVED_RELEASE" = "$STABLE_RELEASE" ] && [ "$FLEET_ROLLBACK_READY" = "1" ] && [ "$READ_ONLY" = "1" ] && [ "$canary_release" = "$STABLE_RELEASE" ] && [ "$current" = "$CURRENT_RELEASE" ]; then
      printf '%s\n' "$STABLE_RELEASE" > "$fleet_state_file"
      printf '%s\n' "boundary_fleet_rollback=ok host=$FLEET_PRIVATE_HOST release=$STABLE_RELEASE"
    else
      printf '%s\n' 'blocked' > "$fleet_state_file"
      printf '%s\n' "boundary_fleet_rollback=failed host=$FLEET_PRIVATE_HOST release=$STABLE_RELEASE"
      exit 1
    fi
    ;;
  health)
    current=$(cat "$fleet_state_file" 2>/dev/null || printf '%s' 'unknown')
    if [ "$current" = "$STABLE_RELEASE" ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "boundary_fleet_health=ok host=$FLEET_PRIVATE_HOST release=$STABLE_RELEASE"
      exit 0
    fi
    printf '%s\n' "boundary_fleet_health=failed host=$FLEET_PRIVATE_HOST release=$STABLE_RELEASE"
    exit 1
    ;;
  *)
    echo "usage: ssh-private-fleet.sh status|rollback|health" >&2
    exit 2
    ;;
esac
EOF_BIN
...
printf '%s\n' '2026.03.22' > "$ROOT_DIR/state/private.canary.release"
printf '%s\n' '2026.03.22' > "$ROOT_DIR/state/private.fleet.release"
printf '%s\n' 'rollback_status=ready'
EOF_BIN
  chmod +x "$workspace_dir/bin/"*.sh
  cat > "$workspace_dir/README.md" <<'EOF_README'
# Remote Boundary Rollback Demo

Use the bastion and private-target helpers in `bin/` to recover the bounded partially landed
boundary rollout. Recovery is healthy only when the bastion tunnel opens first, the private
canary target rolls back second, and the private fleet target rolls back last while all hosts
remain in read-only mode and rollback stays intact.
EOF_README
}
...
')
[ -n "$model" ] || { echo "No installed models available; remote boundary rollback probe cannot run." >&2; exit 1; }
...
create_remote_boundary_rollback_workspace "$tmp_ws"
...
Handle the bounded remote boundary rollback in this workspace. Use `./bin/ssh-bastion.sh`, `./bin/ssh-private-canary.sh`, `./bin/ssh-private-fleet.sh`, and `./bin/rollback.sh`. Make the smallest safe release-config fix so the bastion tunnel opens first, the private canary target rolls back second, the private fleet target rolls back last, rerun the health checks, keep rollback intact, and do not widen beyond this one boundary rollback. End with sections: Outcome, Verification Evidence, Risks, Next Improvement.
EOF_PROMPT
)
...
config_fixed=0
if grep -q '^APPROVED_RELEASE=2026.03.10$' "$tmp_ws/remote/boundary.env" \
  && grep -q '^TUNNEL_READY=1$' "$tmp_ws/remote/boundary.env" \
  && grep -q '^CANARY_ROLLBACK_READY=1$' "$tmp_ws/remote/boundary.env" \
  && grep -q '^FLEET_ROLLBACK_READY=1$' "$tmp_ws/remote/boundary.env" \
...
canary_rollback_ran=$(printf '%s' "$run_json" | jq -r '[.commands[]?.command // ""] | any(contains("bin/ssh-private-canary.sh rollback")) | if . then 1 else 0 end')
...
fleet_rollback_ran=$(printf '%s' "$run_json" | jq -r '[.commands[]?.command // ""] | any(contains("bin/ssh-private-fleet.sh rollback")) | if . then 1 else 0 end')
...
if [ "$timed_out" -eq 0 ] && [ "$config_fixed" -eq 1 ] \
  ...
  && [ "$canary_status_ran" -eq 1 ] && [ "$canary_rollback_ran" -eq 1 ] && [ "$canary_health_ran" -eq 1 ] \
  && [ "$fleet_status_ran" -eq 1 ] && [ "$fleet_rollback_ran" -eq 1 ] && [ "$fleet_health_ran" -eq 1 ] \
...
printf '{"label":"%s","status":"%s","model":"%s","timed_out":%s,"config_fixed":%s,"bastion_status_ran":%s,"bastion_tunnel_ran":%s,"bastion_health_ran":%s,"canary_status_ran":%s,"canary_rollback_ran":%s,"canary_health_ran":%s,"fleet_status_ran":%s,"fleet_rollback_ran":%s,"fleet_health_ran":%s,"bastion_health_ok":%s,"canary_health_ok":%s,"fleet_health_ok":%s,"rollback_referenced":%s,"rollback_intact":%s,"has_outcome":%s,"has_verify":%s,"has_risks":%s,"has_next":%s,"stream_line_count":%s}\n' \
  "$label" "$status" "$model" "$timed_out" "$config_fixed" "$bastion_status_ran" "$bastion_tunnel_ran" "$bastion_health_ran" "$canary_status_ran" "$canary_rollback_ran" "$canary_health_ran" "$fleet_status_ran" "$fleet_rollback_ran" "$fleet_health_ran" "$bastion_health_ok" "$canary_health_ok" "$fleet_health_ok" "$rollback_ref" "$rollback_intact" "$has_outcome" "$has_verify" "$has_risks" "$has_next" "$stream_line_count" > "$json_file"

{
  printf '# Remote Boundary Rollback Probe: %s\n\n' "$label"
  printf -- '- Status: %s\n' "$status"
  printf -- '- Model: %s\n' "$model"
  printf -- '- Timed out: %s\n' "$timed_out"
  ...
  printf -- '- `ssh-private-canary.sh rollback` ran: %s\n' "$canary_rollback_ran"
  printf -- '- `ssh-private-canary.sh health` ran: %s\n' "$canary_health_ran"
  printf -- '- `ssh-private-fleet.sh status` ran: %s\n' "$fleet_status_ran"
  printf -- '- `ssh-private-fleet.sh rollback` ran: %s\n' "$fleet_rollback_ran"
