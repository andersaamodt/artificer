...
DEFAULT_LABEL="service-restart-rollback-probe"
API_SCRIPT="${ARTIFICER_API_SCRIPT:-$SITE_ROOT/cgi/artificer-api}"

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required for system ops probe." >&2
  exit 1
fi

usage() {
  cat <<'EOF_USAGE'
Usage: system-ops-probe.sh [--label NAME] [--prompt TEXT] [--prompt-file PATH]

Runs a live local system-ops probe against a demo service workspace and checks
whether Artificer can diagnose, repair, restart, verify, and keep rollback intact.
EOF_USAGE
}
...
create_service_workspace() {
  workspace_dir=$1
  mkdir -p "$workspace_dir/bin" "$workspace_dir/service" "$workspace_dir/state"
  cat > "$workspace_dir/service/config.env" <<'EOF_CFG'
MODE=degraded
READ_ONLY=0
PORT=18080
EOF_CFG
  cp "$workspace_dir/service/config.env" "$workspace_dir/service/config.env.bak"
  cat > "$workspace_dir/bin/status.sh" <<'EOF_SH'
#!/bin/sh
set -eu
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
. "$ROOT_DIR/service/config.env"
...
EOF_SH
  cat > "$workspace_dir/bin/restart.sh" <<'EOF_SH'
#!/bin/sh
set -eu
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
. "$ROOT_DIR/service/config.env"
if [ "$MODE" = "healthy" ] && [ "$READ_ONLY" = "1" ]; then
  printf '%s\n' "healthy" > "$ROOT_DIR/state/service.status"
  printf '%s\n' "restart_status=ok"
else
  printf '%s\n' "degraded" > "$ROOT_DIR/state/service.status"
...
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
. "$ROOT_DIR/service/config.env"
status=$(cat "$ROOT_DIR/state/service.status" 2>/dev/null || printf '%s' "unknown")
...
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
cp "$ROOT_DIR/service/config.env.bak" "$ROOT_DIR/service/config.env"
printf '%s\n' "rollback_ready" > "$ROOT_DIR/state/rollback.status"
...
Use the scripts in `bin/` to inspect, restart, validate, and roll back the service.
The service is healthy only when `MODE=healthy` and `READ_ONLY=1`.
EOF_README
}
...
')
[ -n "$model" ] || { echo "No installed models available; system ops probe cannot run." >&2; exit 1; }
...
create_service_workspace "$tmp_ws"
...
Diagnose the local demo service in this workspace. Use the provided `bin/status.sh`, `bin/restart.sh`, `bin/health.sh`, and `bin/rollback.sh` helpers. Make the smallest safe config fix so the service restarts cleanly, run the restart and health checks, and keep rollback intact. Do not widen beyond this one service. End with sections: Outcome, Verification Evidence, Risks, Next Improvement.
EOF_PROMPT
)
...
config_fixed=0
if grep -q '^MODE=healthy$' "$tmp_ws/service/config.env" && grep -q '^READ_ONLY=1$' "$tmp_ws/service/config.env"; then
  config_fixed=1
fi

status_ran=$(printf '%s' "$run_json" | jq -r '[.commands[]?.command // ""] | any(contains("bin/status.sh")) | if . then 1 else 0 end')
restart_ran=$(printf '%s' "$run_json" | jq -r '[.commands[]?.command // ""] | any(contains("bin/restart.sh")) | if . then 1 else 0 end')
...
rollback_intact=$([ -x "$tmp_ws/bin/rollback.sh" ] && cmp -s "$baseline_dir/service/config.env.bak" "$tmp_ws/service/config.env.bak" && printf '%s' "1" || printf '%s' "0")
stream_line_count=$(printf '%s\n' "$stream_text" | sed '/^[[:space:]]*$/d' | wc -l | tr -d ' ')

status="fail"
if [ "$timed_out" -eq 0 ] && [ "$config_fixed" -eq 1 ] && [ "$status_ran" -eq 1 ] && [ "$restart_ran" -eq 1 ] && [ "$health_ran" -eq 1 ] && [ "$health_ok" -eq 1 ] && [ "$rollback_ref" -eq 1 ] && [ "$rollback_intact" -eq 1 ] && [ "$has_outcome" -eq 1 ] && [ "$has_verify" -eq 1 ] && [ "$has_risks" -eq 1 ] && [ "$has_next" -eq 1 ]; then
  status="pass"
fi

printf '{"label":"%s","status":"%s","model":"%s","timed_out":%s,"config_fixed":%s,"status_ran":%s,"restart_ran":%s,"health_ran":%s,"health_ok":%s,"rollback_referenced":%s,"rollback_intact":%s,"has_outcome":%s,"has_verify":%s,"has_risks":%s,"has_next":%s,"stream_line_count":%s}\n' \
  "$label" "$status" "$model" "$timed_out" "$config_fixed" "$status_ran" "$restart_ran" "$health_ran" "$health_ok" "$rollback_ref" "$rollback_intact" "$has_outcome" "$has_verify" "$has_risks" "$has_next" "$stream_line_count" > "$json_file"

{
  printf '# System Ops Probe: %s\n\n' "$label"
  printf -- '- Status: %s\n' "$status"
  printf -- '- Model: %s\n' "$model"
  printf -- '- Timed out: %s\n' "$timed_out"
  printf -- '- Config fixed: %s\n' "$config_fixed"
  printf -- '- `status.sh` ran: %s\n' "$status_ran"
  printf -- '- `restart.sh` ran: %s\n' "$restart_ran"
