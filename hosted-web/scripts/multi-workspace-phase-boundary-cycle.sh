...
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-multi-workspace-phase-boundary-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-multi-workspace-phase-boundary-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  multi-workspace-phase-boundary-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  multi-workspace-phase-boundary-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}
...
run_profile() {
  ...
  if [ ! -f "$smoke_json" ]; then
    ...
    {
      printf '# Multi Workspace Phase Boundary %s\n\n' "$label"
      printf -- '- Profile: %s\n' "$profile"
      printf -- '- Smoke rc: %s\n' "$smoke_rc"
...
run_transfer() {
  ...
  {
    printf '# Multi Workspace Phase Boundary Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="multi-workspace-phase-boundary-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="multi-workspace-phase-boundary-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
