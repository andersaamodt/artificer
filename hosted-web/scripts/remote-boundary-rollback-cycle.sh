...
PROBE_SCRIPT="$SCRIPT_DIR/remote-boundary-rollback-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-remote-boundary-rollback-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-remote-boundary-rollback-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  remote-boundary-rollback-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  remote-boundary-rollback-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "remote-boundary-rollback-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  label=$2
  case "$profile" in
    regressions) tasks_file=$DEFAULT_REGRESSIONS ;;
    holdout) tasks_file=$DEFAULT_HOLDOUT ;;
...

  {
    ...
    while IFS=$(printf '\t') read -r task_id prompt; do
      [ -n "$task_id" ] || continue
      total=$((total + 1))
      ...
      printf '%s\n' "$prompt" > "$prompt_file"
      set +e
      probe_output=$(sh "$PROBE_SCRIPT" --label "$row_label" --prompt-file "$prompt_file" 2>&1)
      probe_rc=$?
      set -e
      rm -f "$prompt_file"
      ...
      if [ -n "$row_json" ] && [ -f "$row_json" ]; then
        ...
        row_payload=$(printf '{"label":"%s","status":"fail","probe_rc":%s,"probe_output":"%s"}' \
          "$(printf '%s' "$row_label" | jq -Rr @json | sed 's/^"//; s/"$//')" \
          "$probe_rc" \
          "$(printf '%s' "$probe_output" | jq -Rr @json | sed 's/^"//; s/"$//')")
      fi
  ...
  {
    printf '# Remote Boundary Rollback %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Remote Boundary Rollback Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="remote-boundary-rollback-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=''
    holdout_report=''
    label='remote-boundary-rollback-v1-transfer'
    while [ "$#" -gt 0 ]; do
      case "$1" in
        --regressions-report) regressions_report=${2:-}; shift 2 ;;
        --holdout-report) holdout_report=${2:-}; shift 2 ;;
        --label) label=${2:-}; shift 2 ;;
        -h|--help) usage; exit 0 ;;
        *) echo "Unknown argument: $1" >&2; usage >&2; exit 1 ;;
      esac
