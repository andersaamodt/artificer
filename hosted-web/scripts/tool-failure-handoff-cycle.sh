...
PROBE_SCRIPT="$SCRIPT_DIR/tool-failure-handoff-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-tool-failure-handoff-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-tool-failure-handoff-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  tool-failure-handoff-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  tool-failure-handoff-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "tool-failure-handoff-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_transfer() {
  ...
  {
    printf '# Tool Failure Handoff Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="tool-failure-handoff-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="tool-failure-handoff-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
