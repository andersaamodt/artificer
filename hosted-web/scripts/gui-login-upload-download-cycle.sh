...
PROBE_SCRIPT="$SCRIPT_DIR/gui-login-upload-download-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-gui-login-upload-download-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-gui-login-upload-download-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  gui-login-upload-download-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  gui-login-upload-download-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "gui-login-upload-download-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
...

  {
    ...
    while IFS=$(printf '\t') read -r task_id scenario; do
      [ -n "$task_id" ] || continue
      total=$((total + 1))
      row_label="${label}-${task_id}"
      set +e
      probe_output=$(sh "$PROBE_SCRIPT" --label "$row_label" --scenario "$scenario" 2>&1)
      probe_rc=$?
      set -e
  ...
  {
    printf '# GUI Login Upload Download %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# GUI Login Upload Download Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="gui-login-upload-download-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="gui-login-upload-download-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
