...
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-long-horizon-timeout-coverage-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-long-horizon-timeout-coverage-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  long-horizon-timeout-coverage-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  long-horizon-timeout-coverage-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}
...
run_profile() {
  ...
  aggregate_json="$OUT_DIR/$label.json"
  aggregate_tsv="$OUT_DIR/$label.tsv"
  set +e
  sh "$SMOKE_SCRIPT" --label "$label" --fixtures "$tasks_file" </dev/null >/dev/null 2>&1
  smoke_rc=$?
  set -e

  if [ -f "$aggregate_json" ]; then
    total=$(jq -r '.total // 0' "$aggregate_json")
    passes=$(jq -r '.passes // 0' "$aggregate_json")
    failures=$(jq -r '.failures // 0' "$aggregate_json")
    rows_json=$(jq -c --arg label "$label" --arg profile "$profile" \
      '[.results[] | . + {label:($label + "-" + (.task_id // "unknown")), profile:$profile}]' \
      "$aggregate_json")
  else
    total=0
    passes=0
    failures=1
    rows_json=$(jq -nc --arg label "$label" --arg profile "$profile" --argjson smoke_rc "$smoke_rc" \
      '[{label:$label,profile:$profile,task_id:"aggregate-run",status:"fail",smoke_rc:$smoke_rc}]')
  fi

  pass_rate=$(awk -v p="$passes" -v t="$total" 'BEGIN { if (t == 0) printf "0.0000"; else printf "%.4f", p / t }')
  all_pass=false
  if [ "$smoke_rc" -eq 0 ] && [ "$failures" -eq 0 ] && [ "$total" -gt 0 ]; then
    all_pass=true
  fi

  printf '{"label":"%s","profile":"%s","total":%s,"passes":%s,"failures":%s,"pass_rate":%s,"all_pass":%s,"rows":[%s]}\n' \
    "$label" "$profile" "$total" "$passes" "$failures" "$pass_rate" "$all_pass" "$(printf '%s' "$rows_json" | jq -c '.[]' | paste -sd, -)" > "$summary_json"

  {
    printf '# Long-Horizon Timeout Coverage %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Tasks file: %s\n' "$tasks_file"
    printf -- '- Aggregate TSV: %s\n' "$aggregate_tsv"
    printf -- '- Aggregate JSON: %s\n' "$aggregate_json"
    printf -- '- Passes: %s/%s\n' "$passes" "$total"
    printf -- '- Pass rate: %s\n' "$pass_rate"
    printf -- '- All pass: %s\n' "$all_pass"
  } > "$summary_md"
...
run_transfer() {
  ...
  {
    printf '# Long-Horizon Timeout Coverage Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="long-horizon-timeout-coverage-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="long-horizon-timeout-coverage-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
