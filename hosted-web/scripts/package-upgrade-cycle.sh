...
PROBE_SCRIPT="$SCRIPT_DIR/package-upgrade-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-package-upgrade-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-package-upgrade-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  package-upgrade-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  package-upgrade-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "package-upgrade-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_transfer() {
  ...
  {
    printf '# Package Upgrade Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="package-upgrade-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="package-upgrade-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
