...
PROBE_SCRIPT="$SCRIPT_DIR/diagram-annotation-read-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-diagram-annotation-read-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-diagram-annotation-read-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  diagram-annotation-read-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  diagram-annotation-read-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "diagram-annotation-read-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  {
    printf '# Diagram Annotation Read %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Diagram Annotation Read Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="diagram-annotation-read-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="diagram-annotation-read-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
