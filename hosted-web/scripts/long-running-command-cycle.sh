...
PROBE_SCRIPT="$SCRIPT_DIR/long-running-command-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-long-running-command-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-long-running-command-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  long-running-command-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  long-running-command-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "long-running-command-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_transfer() {
  ...
  {
    printf '# Long-Running Command Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="long-running-command-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="long-running-command-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
