...
PROBE_SCRIPT="$SCRIPT_DIR/gui-visual-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-gui-visual-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-gui-visual-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  gui-visual-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  gui-visual-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "gui-visual-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
...

  {
    ...
    while IFS=$(printf '\t') read -r task_id browser profile_name required_checks; do
      [ -n "$task_id" ] || continue
      total=$((total + 1))
      row_label="${label}-${task_id}"
      set +e
      if [ -n "${required_checks:-}" ]; then
        probe_output=$(sh "$PROBE_SCRIPT" --label "$row_label" --browser "$browser" --profile "$profile_name" --required-checks "$required_checks" 2>&1)
      else
        probe_output=$(sh "$PROBE_SCRIPT" --label "$row_label" --browser "$browser" --profile "$profile_name" 2>&1)
      fi
      probe_rc=$?
      set -e
