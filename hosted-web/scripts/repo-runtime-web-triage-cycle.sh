...
PROBE_SCRIPT="$SCRIPT_DIR/repo-runtime-web-triage-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-repo-runtime-web-triage-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-repo-runtime-web-triage-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  repo-runtime-web-triage-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  repo-runtime-web-triage-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "repo-runtime-web-triage-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
    ...
    while IFS=$(printf '\t') read -r task_id prompt; do
      ...
      if [ -n "$row_json" ] && [ -f "$row_json" ]; then
        ...
        row_payload=$(jq -n \
          ...
          '{label:$label,status:"fail",probe_rc:$probe_rc,probe_output:$output}')
      fi
  ...
  {
    printf '# Repo Runtime Web Triage %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Repo Runtime Web Triage Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="repo-runtime-web-triage-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="repo-runtime-web-triage-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
