...
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-operator-decision-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-operator-decision-holdout-v1.tsv"

usage() {
  cat <<'USAGE'
Usage:
  operator-decision-cycle.sh run [--profile regressions|holdout] [broad-reasoning-cycle run args...]
  operator-decision-cycle.sh transfer [broad-reasoning-cycle transfer args...]

Notes:
  - This is a thin wrapper over broad-reasoning-cycle.sh with operator-decision defaults.
