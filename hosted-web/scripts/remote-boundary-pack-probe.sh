...
DEFAULT_LABEL="remote-boundary-pack-probe"
API_SCRIPT="${ARTIFICER_API_SCRIPT:-$SITE_ROOT/cgi/artificer-api}"

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required for remote boundary pack probe." >&2
  exit 1
fi

usage() {
  cat <<'EOF_USAGE'
Usage: remote-boundary-pack-probe.sh [--label NAME] [--prompt TEXT] [--prompt-file PATH]

Runs a live bounded remote boundary-pack probe against a demo workspace and checks
whether Artificer can open the bastion tunnel, deploy the core boundary pair first,
then deploy the edge boundary pair second, rerun pack verification, and keep rollback intact.
EOF_USAGE
}
...
create_remote_boundary_pack_workspace() {
  workspace_dir=$1
  mkdir -p "$workspace_dir/bin" "$workspace_dir/remote" "$workspace_dir/state" "$workspace_dir/audit"
  cat > "$workspace_dir/remote/boundary-pack.env" <<'EOF_CFG'
...
PACK_STATE=stale
EOF_CFG
  cp "$workspace_dir/remote/boundary-pack.env" "$workspace_dir/remote/boundary-pack.env.bak"
...
subcommand=${1:-}
. "$ROOT_DIR/remote/boundary-pack.env"
state_file="$ROOT_DIR/state/boundary.tunnel"
case "$subcommand" in
  status)
    if [ "$TUNNEL_READY" = "1" ] && [ "$READ_ONLY" = "1" ] && [ "$PACK_STATE" = "ready" ]; then
      printf '%s\n' "boundary_pack_bastion=ready host=$BASTION_HOST"
    else
      printf '%s\n' "boundary_pack_bastion=stale host=$BASTION_HOST"
  ...
  tunnel)
    if [ "$TUNNEL_READY" = "1" ] && [ "$READ_ONLY" = "1" ] && [ "$PACK_STATE" = "ready" ]; then
      printf '%s\n' 'ready' > "$state_file"
      printf '%s\n' "boundary_pack_tunnel=ok host=$BASTION_HOST"
    else
      printf '%s\n' 'blocked' > "$state_file"
      printf '%s\n' "boundary_pack_tunnel=failed host=$BASTION_HOST"
      exit 1
    fi
    ;;
  health)
    ...
    if [ "$tunnel_state" = 'ready' ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "boundary_pack_bastion_health=ok host=$BASTION_HOST"
      exit 0
    fi
    printf '%s\n' "boundary_pack_bastion_health=failed host=$BASTION_HOST"
    exit 1
    ;;
  *)
...
EOF_BIN
  ...
  cat > "$workspace_dir/bin/verify-pack.sh" <<'EOF_BIN'
#!/bin/sh
set -eu
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
...
if [ "$READ_ONLY" = "1" ] && [ "$PACK_STATE" = "ready" ] && [ "$tunnel_state" = 'ready' ] \
  ...
  printf '%s\n' 'boundary_pack_verify=ok'
  exit 0
fi
printf '%s\n' 'boundary_pack_verify=failed'
exit 1
EOF_BIN
...
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
cp "$ROOT_DIR/remote/boundary-pack.env.bak" "$ROOT_DIR/remote/boundary-pack.env"
...
rm -f "$ROOT_DIR/state/boundary.tunnel" "$ROOT_DIR/audit/core-canary.log" "$ROOT_DIR/audit/core-fleet.log" "$ROOT_DIR/audit/edge-canary.log" "$ROOT_DIR/audit/edge-fleet.log"
printf '%s\n' 'rollback_status=ready'
EOF_BIN

  chmod +x "$workspace_dir/bin/"*.sh
  cat > "$workspace_dir/README.md" <<'EOF_README'
# Remote Boundary Pack Demo

Use the bastion helper plus the core and edge private-boundary helpers in `bin/` to recover the bounded remote boundary pack.
The pack is healthy only when the bastion tunnel opens first, the core boundary pair deploys second and third,
the edge boundary pair deploys fourth and fifth, `./bin/verify-pack.sh` passes, and rollback stays intact.
EOF_README
}
...
')
[ -n "$model" ] || { echo "No installed models available; remote boundary pack probe cannot run." >&2; exit 1; }
...
create_remote_boundary_pack_workspace "$tmp_ws"
...
Handle the bounded remote boundary pack in this workspace. Use `./bin/ssh-bastion.sh`, `./bin/ssh-core-canary.sh`, `./bin/ssh-core-fleet.sh`, `./bin/ssh-edge-canary.sh`, `./bin/ssh-edge-fleet.sh`, `./bin/verify-pack.sh`, and `./bin/rollback.sh`. Make the smallest safe shared boundary-pack fix so the bastion tunnel opens first, the core boundary pair deploys before the edge boundary pair, rerun pack verification, keep rollback intact, and do not widen beyond this one remote boundary pack. End with sections: Outcome, Verification Evidence, Risks, Next Improvement.
EOF_PROMPT
)
...
config_fixed=0
if grep -q '^CORE_APPROVED_RELEASE=2026.03.22-core$' "$tmp_ws/remote/boundary-pack.env" \
  && grep -q '^EDGE_APPROVED_RELEASE=2026.03.22-edge$' "$tmp_ws/remote/boundary-pack.env" \
  && grep -q '^TUNNEL_READY=1$' "$tmp_ws/remote/boundary-pack.env" \
  && grep -q '^CORE_CANARY_READY=1$' "$tmp_ws/remote/boundary-pack.env" \
  && grep -q '^CORE_FLEET_READY=1$' "$tmp_ws/remote/boundary-pack.env" \
  && grep -q '^EDGE_CANARY_READY=1$' "$tmp_ws/remote/boundary-pack.env" \
  && grep -q '^EDGE_FLEET_READY=1$' "$tmp_ws/remote/boundary-pack.env" \
  && grep -q '^READ_ONLY=1$' "$tmp_ws/remote/boundary-pack.env" \
  && grep -q '^PACK_STATE=ready$' "$tmp_ws/remote/boundary-pack.env"; then
  config_fixed=1
fi
...
verify_pack_ran=$(printf '%s' "$run_json" | jq -r '[.commands[]?.command // ""] | any(contains("bin/verify-pack.sh")) | if . then 1 else 0 end')
...
rollback_intact=$([ -x "$tmp_ws/bin/rollback.sh" ] \
  && cmp -s "$baseline_dir/remote/boundary-pack.env.bak" "$tmp_ws/remote/boundary-pack.env.bak" \
