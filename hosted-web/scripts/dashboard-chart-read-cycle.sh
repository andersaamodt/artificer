...
PROBE_SCRIPT="$SCRIPT_DIR/dashboard-chart-read-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-dashboard-chart-read-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-dashboard-chart-read-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  dashboard-chart-read-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  dashboard-chart-read-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "dashboard-chart-read-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_transfer() {
  ...
  {
    printf '# Dashboard Chart Read Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="dashboard-chart-read-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="dashboard-chart-read-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
