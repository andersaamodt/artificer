...
PROBE_SCRIPT="$SCRIPT_DIR/browser-image-run-investigation-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-browser-image-run-investigation-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-browser-image-run-investigation-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  browser-image-run-investigation-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  browser-image-run-investigation-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "browser-image-run-investigation-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  {
    printf '# Browser Image Run Investigation %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Browser Image Run Investigation Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="browser-image-run-investigation-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="browser-image-run-investigation-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
