...
PROBE_SCRIPT="$SCRIPT_DIR/standards-grounded-answer-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-standards-grounded-answer-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-standards-grounded-answer-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  standards-grounded-answer-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  standards-grounded-answer-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "standards-grounded-answer-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  {
    printf '# Standards Grounded Answer %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Standards Grounded Answer Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="standards-grounded-answer-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="standards-grounded-answer-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
