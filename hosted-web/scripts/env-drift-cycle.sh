...
PROBE_SCRIPT="$SCRIPT_DIR/env-drift-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-env-drift-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-env-drift-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  env-drift-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  env-drift-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "env-drift-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
