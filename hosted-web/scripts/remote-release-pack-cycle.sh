...
PROBE_SCRIPT="$SCRIPT_DIR/remote-release-pack-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-remote-release-pack-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-remote-release-pack-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  remote-release-pack-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  remote-release-pack-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "remote-release-pack-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_transfer() {
  ...
  {
    printf '# Remote Release Pack Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="remote-release-pack-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="remote-release-pack-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
