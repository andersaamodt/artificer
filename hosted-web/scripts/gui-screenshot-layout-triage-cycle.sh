...
PROBE_SCRIPT="$SCRIPT_DIR/gui-screenshot-layout-triage-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-gui-screenshot-layout-triage-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-gui-screenshot-layout-triage-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  gui-screenshot-layout-triage-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  gui-screenshot-layout-triage-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "gui-screenshot-layout-triage-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  {
    printf '# GUI Screenshot Layout Triage %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# GUI Screenshot Layout Triage Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="gui-screenshot-layout-triage-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="gui-screenshot-layout-triage-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
