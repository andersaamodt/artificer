...
DEFAULT_LABEL="remote-single-host-diagnose-probe"
API_SCRIPT="${ARTIFICER_API_SCRIPT:-$SITE_ROOT/cgi/artificer-api}"

if ! command -v jq >/dev/null 2>&1; then
  echo "jq is required for remote ops probe." >&2
  exit 1
fi

usage() {
  cat <<'EOF_USAGE'
Usage: remote-ops-probe.sh [--label NAME] [--prompt TEXT] [--prompt-file PATH]

Runs a live single-host remote-ops probe against a demo workspace and checks
whether Artificer can inspect, repair, restart, verify, and keep rollback intact.
EOF_USAGE
}
...
create_remote_workspace() {
  ...
  cat > "$workspace_dir/remote/service.env" <<'EOF_CFG'
MODE=degraded
READ_ONLY=0
HOST=demo-app-1
PORT=18080
EOF_CFG
  cp "$workspace_dir/remote/service.env" "$workspace_dir/remote/service.env.bak"
...
subcommand=${1:-}
. "$ROOT_DIR/remote/service.env"
status_file="$ROOT_DIR/state/remote.status"
case "$subcommand" in
  status)
    if [ "$MODE" = "healthy" ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "remote_service=healthy host=$HOST"
    else
      printf '%s\n' "remote_service=degraded host=$HOST"
      printf '%s\n' "expected_fix=MODE=healthy READ_ONLY=1"
    fi
    ;;
  journal)
    if [ "$MODE" = "healthy" ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "journal=clean host=$HOST"
    else
      printf '%s\n' "journal=read-only disabled on $HOST"
    fi
    ;;
  restart)
    if [ "$MODE" = "healthy" ] && [ "$READ_ONLY" = "1" ]; then
      printf '%s\n' "healthy" > "$status_file"
      printf '%s\n' "restart_status=ok host=$HOST"
    else
      printf '%s\n' "degraded" > "$status_file"
      printf '%s\n' "restart_status=degraded host=$HOST"
    fi
    ;;
  health)
    status=$(cat "$status_file" 2>/dev/null || printf '%s' 'unknown')
    if [ "$MODE" = "healthy" ] && [ "$READ_ONLY" = "1" ] && [ "$status" = "healthy" ]; then
      printf '%s\n' "remote_health=ok host=$HOST"
      exit 0
    fi
    printf '%s\n' "remote_health=failed host=$HOST"
    exit 1
    ;;
  *)
    echo "usage: ssh.sh status|journal|restart|health" >&2
    exit 2
    ;;
esac
EOF_SH
...
SCRIPT_DIR=$(CDPATH= cd -- "$(dirname "$0")" && pwd)
ROOT_DIR=$(CDPATH= cd -- "$SCRIPT_DIR/.." && pwd)
cp "$ROOT_DIR/remote/service.env.bak" "$ROOT_DIR/remote/service.env"
printf '%s\n' "rollback_status=ready"
EOF_SH
  chmod +x "$workspace_dir/bin/"*.sh
  cat > "$workspace_dir/README.md" <<'EOF_README'
# Remote Single-Host Demo

Use `bin/ssh.sh` to inspect and operate on the single remote host service.
The remote host is healthy only when `MODE=healthy` and `READ_ONLY=1` in `remote/service.env`.
EOF_README
}
...
')
[ -n "$model" ] || { echo "No installed models available; remote ops probe cannot run." >&2; exit 1; }
...
create_remote_workspace "$tmp_ws"
...
Diagnose the single remote host in this workspace. Use `./bin/ssh.sh status`, `./bin/ssh.sh journal`, `./bin/ssh.sh restart`, `./bin/ssh.sh health`, and `./bin/rollback.sh`. Make the smallest safe remote config fix so the host restarts cleanly, rerun the remote health check, and keep rollback intact. Do not widen beyond this one remote host. End with sections: Outcome, Verification Evidence, Risks, Next Improvement.
EOF_PROMPT
)
...
config_fixed=0
if grep -q '^MODE=healthy$' "$tmp_ws/remote/service.env" \
  && grep -q '^READ_ONLY=1$' "$tmp_ws/remote/service.env"; then
  config_fixed=1
fi

status_ran=$(printf '%s' "$run_json" | jq -r '[.commands[]?.command // ""] | any(contains("bin/ssh.sh status")) | if . then 1 else 0 end')
journal_ran=$(printf '%s' "$run_json" | jq -r '[.commands[]?.command // ""] | any(contains("bin/ssh.sh journal")) | if . then 1 else 0 end')
restart_ran=$(printf '%s' "$run_json" | jq -r '[.commands[]?.command // ""] | any(contains("bin/ssh.sh restart")) | if . then 1 else 0 end')
...
rollback_intact=$([ -x "$tmp_ws/bin/rollback.sh" ] && cmp -s "$baseline_dir/remote/service.env.bak" "$tmp_ws/remote/service.env.bak" && printf '%s' "1" || printf '%s' "0")
stream_line_count=$(printf '%s\n' "$stream_text" | sed '/^[[:space:]]*$/d' | wc -l | tr -d ' ')

status="fail"
if [ "$timed_out" -eq 0 ] && [ "$config_fixed" -eq 1 ] && [ "$status_ran" -eq 1 ] && [ "$journal_ran" -eq 1 ] && [ "$restart_ran" -eq 1 ] && [ "$health_ran" -eq 1 ] && [ "$health_ok" -eq 1 ] && [ "$rollback_ref" -eq 1 ] && [ "$rollback_intact" -eq 1 ] && [ "$has_outcome" -eq 1 ] && [ "$has_verify" -eq 1 ] && [ "$has_risks" -eq 1 ] && [ "$has_next" -eq 1 ]; then
  status="pass"
fi

printf '{"label":"%s","status":"%s","model":"%s","timed_out":%s,"config_fixed":%s,"status_ran":%s,"journal_ran":%s,"restart_ran":%s,"health_ran":%s,"health_ok":%s,"rollback_referenced":%s,"rollback_intact":%s,"has_outcome":%s,"has_verify":%s,"has_risks":%s,"has_next":%s,"stream_line_count":%s}\n' \
  "$label" "$status" "$model" "$timed_out" "$config_fixed" "$status_ran" "$journal_ran" "$restart_ran" "$health_ran" "$health_ok" "$rollback_ref" "$rollback_intact" "$has_outcome" "$has_verify" "$has_risks" "$has_next" "$stream_line_count" > "$json_file"

{
  printf '# Remote Ops Probe: %s\n\n' "$label"
  printf -- '- Status: %s\n' "$status"
  printf -- '- Model: %s\n' "$model"
  printf -- '- Timed out: %s\n' "$timed_out"
  printf -- '- Config fixed: %s\n' "$config_fixed"
  printf -- '- `ssh.sh status` ran: %s\n' "$status_ran"
  printf -- '- `ssh.sh journal` ran: %s\n' "$journal_ran"
  printf -- '- `ssh.sh restart` ran: %s\n' "$restart_ran"
