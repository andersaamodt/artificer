...
PROBE_SCRIPT="$SCRIPT_DIR/terminal-state-recovery-read-probe.sh"
DEFAULT_REGRESSIONS="$SITE_ROOT/tests/fixtures/artificer-terminal-state-recovery-read-regressions-v1.tsv"
DEFAULT_HOLDOUT="$SITE_ROOT/tests/fixtures/artificer-terminal-state-recovery-read-holdout-v1.tsv"
OUT_DIR="$ARTIFICER_ASSAY_REPORTS_DIR"

usage() {
  cat <<'EOF_USAGE'
Usage:
  terminal-state-recovery-read-cycle.sh run [--profile regressions|holdout] [--label LABEL]
  terminal-state-recovery-read-cycle.sh transfer --regressions-report PATH --holdout-report PATH [--label LABEL]
EOF_USAGE
}

[ -x "$PROBE_SCRIPT" ] || {
  echo "terminal-state-recovery-read-probe.sh is not executable: $PROBE_SCRIPT" >&2
  exit 1
}
...
run_profile() {
  ...
  {
    printf '# Terminal State Recovery Read %s\n\n' "$label"
    printf -- '- Profile: %s\n' "$profile"
    printf -- '- Total: %s\n' "$total"
    printf -- '- Passes: %s\n' "$passes"
    printf -- '- Failures: %s\n' "$failures"
...
run_transfer() {
  ...
  {
    printf '# Terminal State Recovery Read Transfer %s\n\n' "$label"
...
case "$command_name" in
  run)
    ...
    if [ -z "$label" ]; then
      label="terminal-state-recovery-read-v1-$profile"
    fi
  ...
  transfer)
    regressions_report=""
    holdout_report=""
    label="terminal-state-recovery-read-v1-transfer"
    while [ $# -gt 0 ]; do
      case "$1" in
        --regressions-report)
