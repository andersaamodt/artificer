# action: dictate_start
    ...
    requested_started_ms=$(trim "$(param "requested_started_ms")")
    ...
    if dictation_live_has_active_capture; then
      ...
      start_ms=$now_ms
      case "$requested_started_ms" in
        ""|*[!0-9]*)
          ;;
        *)
          max_backfill_ms=2000
          min_allowed_ms=$((now_ms - max_backfill_ms))
          if [ "$requested_started_ms" -lt "$min_allowed_ms" ]; then
            start_ms=$min_allowed_ms
          elif [ "$requested_started_ms" -le "$now_ms" ]; then
            start_ms=$requested_started_ms
          fi
          ;;
      esac
      case "$capture_started_ms" in
        ""|*[!0-9]*)
          ;;
        *)
          if [ "$start_ms" -lt "$capture_started_ms" ]; then
            start_ms=$capture_started_ms
          fi
          ;;
      esac
      ...
      printf '%s\n' "$start_ms" > "$session_dir/started_ms"
      printf '%s\n' "recording" > "$session_dir/status"
      printf '{"success":true,"session":{"id":"%s","status":"recording","backend":"%s"}}\n' \
...
EOF
    ...
    printf '%s\n' "$capture_started_ms" > "$session_dir/started_ms"
    printf '%s\n' "recording" > "$session_dir/status"

    printf '{"success":true,"session":{"id":"%s","status":"recording","backend":"%s","backend_label":"%s"}}\n' \
