# action: queue_take
    ...
    item_reasoning_effort=$(queue_meta_reasoning_effort_from_file "$running_meta_file")
    ...
    item_reasoning_effort_json=$(json_escape "$item_reasoning_effort")
    ...
    printf '{"success":true,"busy":false,"has_item":true,"item":{"id":"%s","prompt":"%s","attachments":%s,"run_mode":"%s","assistant_mode_id":"%s","compute_budget":"%s","reasoning_effort":"%s","command_exec_mode":"%s","permission_mode":"%s","reflexive_knowledge":"%s","self_actuation":"%s","programmer_review":"%s","programmer_review_rounds":"%s","explicit_skill_ids":%s},"queue_pending":%s,"queue_running":1,"queue_done":0,"queue_first_id":"%s","queue_last_status":"running"}\n' \
      "$item_id_json" "$prompt_json" "$attachments_json" "$item_run_mode_json" "$item_assistant_mode_json" "$item_compute_budget_json" "$item_reasoning_effort_json" "$item_command_exec_mode_json" "$item_permission_mode_json" "$item_reflexive_knowledge_json" "$item_self_actuation_json" "$item_programmer_review_json" "$item_programmer_review_rounds_json" "$explicit_skills_json" "$queue_pending" "$queue_first_id_json"
    exit 0
