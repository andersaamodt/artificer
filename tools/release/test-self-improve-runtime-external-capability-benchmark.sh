...
[ "$(json_query "$runtime_json" 'data["counts"].get("capability_benchmark_external_compares")')" = "1" ] || fail "runtime counts should include external benchmark comparisons"
...
[ "$(json_query "$evidence_json" 'data["counts"].get("capability_benchmark_external_compares")')" = "1" ] || fail "evidence bundle counts should include external benchmark comparisons"
