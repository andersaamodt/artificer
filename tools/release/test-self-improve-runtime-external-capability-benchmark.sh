...
{"label":"20260401-candidate-vs-frontier","external_baseline":{"name":"Frontier Reference","kind":"model","model":"gpt-5.4","notes":"reference workflow"},"external_label":"20260401-frontier","candidate_label":"20260401-candidate","recommendation":"external-still-ahead","candidate_beats_external":false,"deltas":{"overall_score":-6.4,"coverage_ratio":0.0,"critical_failures":0,"high_risk_family_count":0},"candidate_gap_families":[{"id":"teaching_reassessment","score_delta":-14.0,"candidate_score":74.0,"external_score":88.0,"candidate_critical":false,"candidate_gate_pass":true,"candidate_weak_reason":"score-below-threshold","external_risk":"low"},{"id":"research_integration","score_delta":-12.0,"candidate_score":77.0,"external_score":89.0,"candidate_critical":true,"candidate_gate_pass":true,"candidate_weak_reason":"external-gap","external_risk":"low"}],"candidate_lead_families":[{"id":"planning_architecture","score_delta":2.0,"candidate_score":90.0,"external_score":88.0,"candidate_critical":true,"candidate_gate_pass":true,"candidate_weak_reason":"","external_risk":"low"}]}
EOF_JSON
...
[ "$(json_query "$runtime_json" 'data["capability_benchmark"]["persistent_external_gaps"][0].get("id")')" = "research_integration" ] || fail "runtime signals should rank critical persistent external gaps first"
...
[ "$(json_query "$runtime_json" 'data["capability_benchmark"]["persistent_external_gaps"][0].get("trend_direction")')" = "worsening" ] || fail "runtime signals should expose worsening persistent external gap direction"
[ "$(json_query "$runtime_json" 'data["capability_benchmark"]["persistent_external_gaps"][0].get("close_rate_per_compare")')" = "-3.0" ] || fail "runtime signals should expose persistent external gap close rate"
[ "$(json_query "$runtime_json" 'data["capability_benchmark"]["persistent_external_gaps"][1].get("trend_direction")')" = "closing" ] || fail "runtime signals should expose closing persistent external gap direction"
[ "$(json_query "$runtime_json" '"research_integration" in data["capability_benchmark"].get("worsening_persistent_external_gap_family_ids", [])')" = "true" ] || fail "runtime signals should expose worsening persistent external gap ids"
[ "$(json_query "$runtime_json" '"teaching_reassessment" in data["capability_benchmark"].get("closing_persistent_external_gap_family_ids", [])')" = "true" ] || fail "runtime signals should expose closing persistent external gap ids"
...
[ "$(json_query "$evidence_json" 'data["runtime_signals"]["capability_benchmark"]["persistent_external_gaps"][0].get("id")')" = "research_integration" ] || fail "evidence bundle should carry persistent external gap ranking"
[ "$(json_query "$evidence_json" 'data["runtime_signals"]["capability_benchmark"]["persistent_external_gaps"][0].get("trend_direction")')" = "worsening" ] || fail "evidence bundle should carry persistent external gap trend direction"
...
printf '%s\n' "ok self-improve runtime external capability benchmark: runtime signals and evidence bundle expose external baseline compares, persistent gap trends, and external adapter metadata"
