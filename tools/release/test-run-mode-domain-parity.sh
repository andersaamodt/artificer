...
for mode_name in $policy_mode_list; do
  if ! grep -Fq "        $mode_name)" "$run_part_tuning_file"; then
    printf '%s\n' "run-part-001 missing mode tuning branch: $mode_name" >&2
    exit 1
  fi
...
if ! awk '
  ...
  in_mode && /reasoning_effort="extra-high"/ { mode_ok=1 }
  END { exit mode_ok ? 0 : 1 }
' "$run_part_tuning_file"; then
  printf '%s\n' "run-part-001 pentest branch must enforce extra-high reasoning effort" >&2
  exit 1
fi

if ! awk '
  /^[[:space:]]*security-audit\)/ { in_mode=1; next }
  in_mode && /^[[:space:]]*[a-z-]+\)/ { in_mode=0 }
  in_mode && /reasoning_effort="extra-high"/ { mode_ok=1 }
  END { exit mode_ok ? 0 : 1 }
' "$run_part_tuning_file"; then
  printf '%s\n' "run-part-001 security-audit branch must enforce extra-high reasoning effort" >&2
  exit 1
fi
